...
- **Aggregate usage counts:** enabled by default on configured production deployments when no preference has been saved. Turn them off in Configuration; existing saved opt-outs and browser DNT/GPC signals are respected. Only fixed action names are sent to a same-origin endpoint. Supabase stores a count per action per UTC day—no addresses, names, domains, user IDs, session IDs, cookies or fingerprint hashes.
