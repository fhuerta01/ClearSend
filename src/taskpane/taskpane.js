...
window.toggleSection = function(contentId, headerElement) {
    ...
    if (content) {
        ...
        if (isHidden) {
            ...
            if (btn) btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="12" height="12" fill="currentColor"><path d="M233.4 105.4c12.5-12.5 32.8-12.5 45.3 0l192 192c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L256 173.3 86.6 342.6c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3l192-192z"/></svg>';
        } else {
            content.style.display = 'none';
            content.classList.add('collapsed');
            if (btn) btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="12" height="12" fill="currentColor"><path d="M233.4 406.6c12.5 12.5 32.8 12.5 45.3 0l192-192c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L256 338.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l192 192z"/></svg>';
        }
    } else {
...
async function initializeClearSend() {
    try {
        showLoadingOverlay('Initializing ClearSend...');
...
function setupRecipientChangeListeners() {
    try {
        ...
        try {
            ...
            if (item.to && item.to.addHandlerAsync && Office.EventType && Office.EventType.RecipientsChanged) {
                // Listen for To recipients changes
                item.to.addHandlerAsync(Office.EventType.RecipientsChanged, onRecipientsChanged);
                // Listen for CC recipients changes
                item.cc.addHandlerAsync(Office.EventType.RecipientsChanged, onRecipientsChanged);
                // Listen for BCC recipients changes
                item.bcc.addHandlerAsync(Office.EventType.RecipientsChanged, onRecipientsChanged);
...
function startRecipientPolling() {
    ...
    ClearSend.recipientPollingInterval = setInterval(checkForChanges, CONFIG.RECIPIENT_POLLING_INTERVAL_MS);
    ...
    try {
        ...
        if (result.status === 'success') {
            try {
                ...
                const currentCount = currentRecipients.to.length + currentRecipients.cc.length + currentRecipients.bcc.length;
                ...
                if (filteredByOutlook > 0) {
                    showToast(`Sorted ${sortAction?.processed || 0} recipients (${filteredByOutlook} invalid entries removed by Outlook)`, 'warning');
                } else {
                ...
                showToast('Sort completed but failed to update recipients: ' + updateError.message, 'warning');
    ...
    try {
        ...
        if (result.status === 'success') {
            try {
                ...
                if (filteredByOutlook > 0) {
                    showToast(`Removed ${dedupeAction?.duplicatesFound || 0} duplicates (${filteredByOutlook} invalid entries removed by Outlook)`, 'warning');
                } else {
                ...
                showToast('Deduplication completed but failed to update recipients: ' + updateError.message, 'warning');
    ...
    try {
        ...
        if (result.status === 'success') {
            try {
                ...
                if (filteredByOutlook > 0) {
                    showToast(`Validated ${validateAction?.processed || 0} recipients: removed ${removedByValidation} invalid (${filteredByOutlook} additional entries removed by Outlook)`, 'warning');
                } else {
                    showToast(`Validated ${validateAction?.processed || 0} recipients: removed ${removedByValidation} invalid`, 'success');
                }
                ...
                showToast('Validation completed but failed to update recipients: ' + updateError.message, 'warning');
...
async function handleClean() {
    try {
        showProgress('Cleaning recipients...');
        ...
        if (enabledSteps.length === 0) {
            showToast('No cleaning steps are enabled. Please enable some features in Configuration tab.', 'warning');
            return;
        }
        ...
        const internalDomains = ClearSend.settings.internalDomains || [];
        ...
        if (enabledSteps.includes('validate')) {
            ...
            invalidCount = (processedTo.length + processedCc.length + processedBcc.length) -
        ...
        if (enabledSteps.includes('prioritizeInternal')) {
            ...
            processedBcc = prioritizeInternalRecipients(processedBcc, internalDomains, sortAlphabetically);
        ...
        // Build comprehensive result message for toast
        const toastParts = [`${totalOriginal} addresses processed`];

        if (enabledSteps.includes('sort') || enabledSteps.includes('prioritizeInternal')) {
            toastParts.push('sorted');
        }
        if (duplicateCount > 0) {
            toastParts.push(`${duplicateCount} de-duplicated`);
        }
        if (invalidCount > 0) {
            toastParts.push(`${invalidCount} invalid removed`);
        }
        if (externalRemovedCount > 0) {
            toastParts.push(`${externalRemovedCount} external removed`);
        }
        if (enabledSteps.includes('prioritizeInternal')) {
            toastParts.push('internals first');
        }

        const toastMessage = toastParts.join(', ');
...
async function handleRefresh() {
    try {
        ...
        if (summaryRefreshBtn) {
            summaryRefreshBtn.classList.add('refreshing');
        }
        ...
        // Remove spinning animation from refresh buttons
        const summaryRefreshBtn = document.getElementById('summaryRefreshBtn');
        if (summaryRefreshBtn) {
            summaryRefreshBtn.classList.remove('refreshing');
        }
        hideProgress();
    }
}
    ...
    try {
        ...
        const removeFirstOccurrence = (recipients, targetAddress) => {
            ...
            return recipients.filter(r => {
                ...
                const displayEmail = r.displayName ? `${r.displayName} <${r.emailAddress}>` : r.emailAddress;
    ...
    return new Promise((resolve, reject) => {
        ...
        field.setAsync(recipients, (result) => {
            if (result.status !== Office.AsyncResultStatus.Succeeded) {
                reject(new Error(`Failed to update ${fieldName} recipients: ${result.error?.message || 'Unknown error'}`));
                return;
            }
            resolve();
        });
...
async function updateRecipientsDirectly(toRecipients, ccRecipients, bccRecipients) {
    try {
        ...
        // Validate inputs
        if (!Array.isArray(toRecipients) || !Array.isArray(ccRecipients) || !Array.isArray(bccRecipients)) {
...
function getRecipientsAsync(field, fieldName) {
    return new Promise((resolve, reject) => {
        ...
        field.getAsync((result) => {
            if (result.status !== Office.AsyncResultStatus.Succeeded) {
                reject(new Error(`Failed to get ${fieldName} recipients: ${result.error?.message || 'Unknown error'}`));
                return;
            }
                ...
                return recipientList
                    .map((r, index) => {
                        try {
                            ...
                            // Handle malformed inputs (less than MIN_EMAIL_LENGTH and no @)
                            if (emailAddress.length <= CONFIG.MIN_EMAIL_LENGTH && !emailAddress.includes('@')) {
            ...
            try {
                ...
                item.to.setAsync(toRecipients, (toResult) => {
                    if (toResult.status !== Office.AsyncResultStatus.Succeeded) {
                        ...
                        reject(new Error('Failed to update To recipients: ' + (toResult.error?.message || 'Unknown error')));
                        return;
                    }
                    ...
                    item.cc.setAsync(ccRecipients, (ccResult) => {
                        if (ccResult.status !== Office.AsyncResultStatus.Succeeded) {
                            ...
                            reject(new Error('Failed to update CC recipients: ' + (ccResult.error?.message || 'Unknown error')));
                            return;
                        }
                        ...
                        item.bcc.setAsync(bccRecipients, (bccResult) => {
                            if (bccResult.status !== Office.AsyncResultStatus.Succeeded) {
                                ...
                                reject(new Error('Failed to update BCC recipients: ' + (bccResult.error?.message || 'Unknown error')));
                                return;
                            }
...
async function callOrchestrator(recipients, enabledSteps, attempt = 1) {
    ...
    if (!Array.isArray(recipients.to) || !Array.isArray(recipients.cc) || !Array.isArray(recipients.bcc)) {
    ...
    const payload = {
        ...
        userSettings: {
            ...
            internalDomains: ClearSend.settings.internalDomains || [],
...
function updateSettings() {
    const enabledSteps = [];
    if (document.getElementById('sortCheck').checked) enabledSteps.push('sort');
    if (document.getElementById('dedupeCheck').checked) enabledSteps.push('dedupe');
    if (document.getElementById('validateCheck').checked) enabledSteps.push('validate');
    if (document.getElementById('prioritizeInternalCheck').checked) enabledSteps.push('prioritizeInternal');
    if (document.getElementById('removeExternalCheck').checked) enabledSteps.push('removeExternal');
...
function updateSettingsUI() {
    ...
    document.getElementById('dedupeCheck').checked = ClearSend.settings.enabledSteps.includes('dedupe');
    document.getElementById('validateCheck').checked = ClearSend.settings.enabledSteps.includes('validate');
    document.getElementById('prioritizeInternalCheck').checked = ClearSend.settings.enabledSteps.includes('prioritizeInternal');
    document.getElementById('removeExternalCheck').checked = ClearSend.settings.enabledSteps.includes('removeExternal');
}

// Tab Management
function switchTab(tabName) {
    // Update tab buttons (only if they exist - may not exist if tab navigation is hidden)
    ...
    // Show/hide footers based on tab
...
function expandSection(sectionName) {
    ...
    if (content && content.classList.contains('collapsed')) {
        content.classList.remove('collapsed');
        btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="12" height="12" fill="currentColor"><path d="M233.4 105.4c12.5-12.5 32.8-12.5 45.3 0l192 192c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L256 173.3 86.6 342.6c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3l192-192z"/></svg>';
    }
}
...
function collapseSection(sectionName) {
    ...
    if (content && !content.classList.contains('collapsed')) {
        content.classList.add('collapsed');
        btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="12" height="12" fill="currentColor"><path d="M233.4 406.6c12.5 12.5 32.8 12.5 45.3 0l192-192c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L256 338.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l192 192z"/></svg>';
    }
}

// Utility Functions
function getEnabledSteps() {
    const result = ClearSend.settings.enabledSteps.filter(step => {
...
function renderInternalDomains() {
    ...
    ClearSend.settings.internalDomains.forEach((domain, index) => {
        ...
        domainItem.innerHTML = `
            <input type="text" class="domain-input" value="${domain}" data-index="${index}" />
...
function updateProcessingOptionsSummary() {
    ...
    if (enabledSteps.includes('sort')) options.push('Sort');
    if (enabledSteps.includes('dedupe')) options.push('De-duplicate');
    if (enabledSteps.includes('validate')) options.push('Validate');
    if (enabledSteps.includes('removeExternal')) options.push('Remove external');
    if (enabledSteps.includes('prioritizeInternal')) options.push('Internals first');
...
function getEmailStatus(address) {
    ...
    const hasBasicEmailFormat = hasAt && email.split('@').length === 2;

    if (!hasBasicEmailFormat) {
        return { status: 'invalid', circle: 'red', label: 'Invalid' };
    }
    ...
    // Check if domain matches internal domains
    const parts = email.split('@');
    if (parts.length !== 2) {
        return { status: 'invalid', circle: 'red', label: 'Invalid' };
    }
...
function populateFieldContent(field, addresses) {
    ...
    addresses.forEach((address) => {
        ...
        item.innerHTML = `
            ${circleHTML}
            <span class="recipient-email" title="${address}" data-full-email="${address}">${address}</span>
            ...
            <button class="recipient-delete" data-field="${field}" data-address="${encodeURIComponent(address)}" title="Remove recipient">
...
window.toggleField = function(field) {
    ...
    const btn = document.getElementById('toggle' + field.charAt(0).toUpperCase() + field.slice(1) + 'Btn');
